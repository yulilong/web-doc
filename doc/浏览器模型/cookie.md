...
可以打开Google Chrome浏览器的开发者工具，点击Network选择一个请求，在Request Headers里面可以看见Cookie字段，里面有发送的cookie值。
...
### 5.1 在Chrome浏览器中查看网站cookie

1. 打开开发者工具(View-> Developer -> Developer Tools)
2. 选择Application，在左边有个：Storage -> Cookies, 选择当前的网址点击就能看见Cookie

### 5.2 创建、修改cookie

使用document.cookie来创建修改cookie，如果没该值，那么创建，如果存在则修改：

```javascript
// 打开Google Chrome浏览器开发者工具，找一个网站，在控制台中测试
document.cookie = 'name=jack'	// "name=jack", 由于没有name，所以会创建各一个cookie
document.cookie					// "name=jack"
document.cookie = 'name=make'	// "name=make" 此时修改一个cookie的值
// 设置cookie的同事设置一个过期时间
document.cookie = "foo=bar; expires=Fri, 31 Dec 2020 23:59:59 GMT";
// 设置 Domain，Path
document.cookie = "qq=ww; domain=jirengu.com; path=/sulaleyogo"
```
...
注意：如果设置了HttpOnly，则无法读取到cookie的值。
...
注意：如果设置了HttpOnly，则无法删除cookie的值。



## 参考资料

[Cookie 阮一峰旧网址](http://javascript.ruanyifeng.com/bom/cookie.html)

[Cookie 阮一峰新网址](https://wangdoc.com/javascript/bom/cookie.html#documentcookie)
